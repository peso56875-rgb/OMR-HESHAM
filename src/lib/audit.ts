...
export const parseTarget = (path: string): { resource: string; id: string | null } => {
  ...
  // آخر مقطع يُعتبر معرّفًا لو لم يكن كلمة فعل معروفة
  const last = rest[rest.length - 1] || ''
  const verbs = ['status', 'toggle', 'approve', 'reject', 'publish', 'feature', 'bulk', 'import']
  const id = rest.length > 1 && !verbs.includes(last) ? last : null
  return { resource, id }
}
