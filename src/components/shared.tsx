  ...
  return <>
    ...
    <aside class="mobile-drawer" id="mobile-drawer" aria-hidden="true">
      ...
      <nav>
        ...
        {Object.entries(routeNames).filter(([h]) => !['/campaigns', '/cases', '/quran', '/zakat-calculator', '/volunteer-portal', '/notifications', '/login', '/profile', '/dashboard'].includes(h)).slice(0, 8).map(([href, label]) => <a href={href}>{label}<i class="fa-solid fa-arrow-left"></i></a>)}
    ...
    <section class="footer-main">
      <article class="footer-story">
        ...
        <div class="legal-strip">
          <span class="licensed">{icon('fa-shield-heart')} مؤسسة أهلية مشهرة — خاضعة لإشراف وزارة التضامن الاجتماعي</span>
...
export function Layout({ children, title = 'مؤسسة الدكتور عمر هشام الخيرية', description = 'عطاء مستمر لتنمية الإنسان والمجتمع', user, showFooter = true, pageType = 'public', image }: { children: any, title?: string, description?: string, user?: UserSession, showFooter?: boolean, pageType?: 'public' | 'auth' | 'dashboard', image?: string }) {
  ...
  return <html lang="ar" dir="rtl"><head>
    ...
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
      ...
      <script src="/static/notifications.js?v=3.2"></script>
    </body></html>
}
