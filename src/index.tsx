...
function Layout({children,title='مؤسسة الدكتور عمر هشام الخيرية',description='عطاء مستمر لتنمية الإنسان والمجتمع',user}:{children:any,title?:string,description?:string,user?:any}){
  return <html lang="ar" dir="rtl"><head>
    ...
    <Header user={user}/><main>{children}</main><Footer/>
      ...
      <div class="news-grid">
        {renderNews.slice(0,3).map((n,i)=> {
          ...
          return <article class={`news-card reveal ${i===0?'featured':''}`}>
            <div class="news-art">{icon(ic)}<span>{cat}</span></div>
            <div>
              <time>{date}</time>
              <h3>{title}</h3>
              <p>{excerpt}</p>
...
function Login({ firebaseConfig }: { firebaseConfig: any }){
  return <Layout title="تسجيل الدخول | مؤسسة الدكتور عمر هشام">
    <section class="auth-page">
      ...
      <div class="auth-form-container" style="background:var(--surface); border:1px solid var(--border); padding:2.5rem; border-radius:24px; box-shadow:var(--sh-xs); max-width:460px; width:100%">
        ...
        <a href="/" class="primary-btn" style="background:transparent;color:var(--text);border:1px solid var(--border);width:100%;text-align:center;display:flex; align-items:center; justify-content:center; height:50px; border-radius:12px">العودة للرئيسية</a>
      </div>
    </section>
...
function Dashboard({ view, data, user }: { view: string, data: any, user: any }){
  ...
  return <Layout user={user} title="لوحة التحكم | مؤسسة الدكتور عمر هشام">
    <section class="dashboard-wrap">
      <aside class="dash-sidebar">
        <div class="dash-brand">
          <img src="/static/foundation-logo.png"/>
          <span>لوحة التحكم</span>
        </div>
        {sideMenu.map((n)=><a class={view === n[2] ? 'active' : ''} href={`/dashboard?view=${n[2]}`}>{icon(n[0])}<span>{n[1]}</span></a>)}
      </aside>
      ...
      <div class="dash-main">
        <header>
          ...
          <a href="/api/auth/logout" class="icon-btn" title="تسجيل الخروج" style="background:#ff6b6b; color:#fff">{icon('fa-right-from-bracket')}</a>
        </header>
  ...
  return <>
    ...
    <section class="section-pad" style="padding-top:2rem">
      <form action="/api/campaigns/add" method="post" style="background:var(--surface); border:1px solid var(--border); padding:2rem; border-radius:16px; max-width:600px; display:flex; flex-direction:column; gap:1.2rem">
        ...
        <label>رابط الصورة<input name="image_url" placeholder="https://..."/></label>
    ...
    <script dangerouslySetInnerHTML={{__html: `
      document.querySelectorAll('.role-toggle-btn').forEach(function(btn) {
        btn.addEventListener('click', async function() {
          ...
          if (!confirm('هل تريد ' + action + ' عن ' + userName + '?')) return;
